...
public class Bezier3rdInterpolate : Interpolator
{
    public override string Name => "Bezier3rd";
    public override void Run (List<GameObject> candidate, uint num)
    {
        if (candidate.Count < 3) return;
        ...
        for (int i = 1; i <= num; i++)
        {
            ...
            for (int j = 0; j < candidate.Count - 3; j++)
            {
                ...
                Vector3 p4 = candidate[j + 3].transform.position;

                Vector3 p12 = Vector3.Lerp(p1, p2, ratio);
                Vector3 p23 = Vector3.Lerp(p2, p3, ratio);
                Vector3 p34 = Vector3.Lerp(p3, p4, ratio);

                Vector3 p123 = Vector3.Lerp(p12, p23, ratio);
                Vector3 p234 = Vector3.Lerp(p23, p34, ratio);

                Vector3 p = Vector3.Lerp(p123, p234, ratio);
